  ...
  if(any(
    ...
    variableGroupIndex = (
      if(finalDataFrame$variable[i] %in% c("education","foreign-born","language")) {
        ...
        # economic
        2
      } else if(finalDataFrame$variable[i] %in% c("housingTenure","vehiclesPerHousingUnit")) {
        # housing
        3
...
for(i in 1:numberOfRows) {
  if(any(
    ...
    if(regionLevel == "ward") {
      if(variable %in% c("belowPovertyLevel","medianAge","medianHouseholdIncome","unemploymentRate")) {
...
for(i in 1:numberOfRows) {
  if(any(
    ...
    if(variable %in% c("housingTenure")) {
      if(year <= 2014) {
...
for(i in 1:numberOfRows) {
  if(any(
    ...
    finalDataFrame$variable[i] %in% c("airQualityIndex","medianAge","medianHouseholdIncome")
  )) {
    NULL
  } else {
    # i = 680
...
for(i in 1:numberOfRows) {
  ...
  } else if(finalDataFrame$variable[i] %in% c("belowPovertyLevel")) {
    # variables like these that get the raw percent pulled in need corrections
    if(is.na(finalDataFrame$count[i])) {
      finalDataFrame$rate[i] = finalDataFrame$count[i]
    } else if(finalDataFrame$count[i] < 0.5) {
      finalDataFrame$rate[i] = finalDataFrame$count[i] * 100
