...
for(i in 1:numberOfRows) {
  finalDataFrame$fileName[i] = (
    if(finalDataFrame$region[i] == 16) {
      fileNames$dc[finalDataFrame$year[i] - 2012]
    } else if(finalDataFrame$region[i] <= 8) {
...
for(i in 1:numberOfRows) {
  if(any(
    finalDataFrame$region[i] %in% c(9:15,17),
    ...
    variableGroupIndex = (
      if(finalDataFrame$variable[i] %in% c("education","foreign-born","language")) {
        ...
        # economic
        2
      } else if(finalDataFrame$variable[i] %in% c("housingTenure","rentGreaterThan35PercentOfHouseholdIncome",
                                                  "vehiclesPerHousingUnit")) {
        # housing
        3
      } else if(finalDataFrame$variable[i] %in% c("race","medianAge","ethnicity")) {
        # demographic
        4
      } else {
...
for(i in 1:numberOfRows) {
  if(any(
    ...
    regionLevel = (if(finalDataFrame$region[i] == 16) {
      "dc"
    } else {
      "ward"
    })
    ...
    if(regionLevel == "ward") {
      if(variable %in% c("belowPovertyLevel","medianAge","medianHouseholdIncome",
...
# populating VA/MD counties from Census Bureau API and EPA API
#####
...
for(i in 1:numberOfRows) {
  if(any(
    finalDataFrame$region[i] %in% c(1:8,16,17),
    finalDataFrame$variable[i] %in% c("airQualityIndex")
  )) {
    NULL
  } else {
...
finalDataFrame %>%
  filter(is.na(count), variable != "airQualityIndex", region != 17) %>%
  filter(!all(variable == "healthInsuranceCoverage", year == 2013, region <= 8))
#####
...
# calculating DMV Catchment Area values (region 17)
#####
# sum regions 1:15
for(i in 1:numberOfRows) {
  if(any(
    finalDataFrame$region[i] != 17,
    finalDataFrame$variable[i] %in% c("airQualityIndex","belowPovertyLevel","medianAge","medianHouseholdIncome",
                                      "rentGreaterThan30PercentOfHouseholdIncome")
  )) {
    NULL
  } else {
    ...
    finalDataFrame$count[i] = finalDataFrame %>%
      filter(variable == variableFilter, year == yearFilter, category == categoryFilter, region <= 15) %>%
      select(count) %>%
      sum()
...
for(i in 1:numberOfRows) {
  if(finalDataFrame$variable[i] %in% c("airQualityIndex","medianAge","medianHouseholdIncome")) {
...
setwd("C:\\Users\\derek.funk\\Desktop\\MSDS\\Capstone\\Cancer_Data_Visualizer\\Data\\Pre_Processing\\preProcessing_createNonCancerMasterDataFile")
source(file = "countyHealthRankings/countyHealthRankings_v3.R")
